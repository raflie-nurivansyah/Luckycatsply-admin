...
const CATEGORIES: { key: ItemCategory; icon: string }[] = [
  { key: "Clothes",      icon: "👕" },
  { key: "Mens",         icon: "👔" },
  { key: "Women",        icon: "👗" },
  { key: "Pants",        icon: "👖" },
...
function FrameGrid({ count, slots, active, onSelect }: {
  ...
  // For count=3 and 5: last item spans full width
            ...
            <div className="flex flex-wrap gap-2">
              {CATEGORIES.map((c) => (
                <button key={c.key} type="button" onClick={() => setCategory(c.key)}
                  ...
                  {c.icon} {tr(CAT_LABELS[c.key] as any, lang)}
                </button>
              ))}
            </div>
          </div>
...
// ── Main Marketing Tab ────────────────────────────────────────────────────────
export function MarketingTab({ lang }: { lang: Lang }) {
              ...
              <div className="p-3 space-y-2">
                ...
                <div className="flex gap-1.5 flex-wrap">
                  ...
                  <span className="px-2 py-0.5 rounded-full" style={{ background: "#e0e7ff", color: "#3730a3", fontSize: "0.6rem", fontWeight: 600 }}>
                    {CATEGORIES.find((c) => c.key === item.category)?.icon} {tr(CAT_LABELS[item.category] as any, lang)}
                  </span>
                </div>
